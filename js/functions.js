...
const CreateBlogListing = () => {
	...
	request.onload = function () {
		if (request.readyState === XMLHttpRequest.DONE && request.status === 200) {
			...
			const cm = document.getElementById('draw')
			console.log(data)
...
const CreateBlogEntry = () => {
	...
	client.onreadystatechange = function () {
		if (client.readyState === XMLHttpRequest.DONE) {
			...
			const backbutton = "<a href='index.html' alt='Return to blog listing'><img id='backbutton' src='lar.svg'></a>"
			if (status === 0 || (status >= 200 && status < 400)) {
				// Begin dealing with YAML information headers.
				const cached_lex = marked.Lexer.lex
				let headerinfo
				marked.Lexer.lex = function (text, options) {
					const parsed = yamlFront.loadFront(text)
				...
				if (headerinfo.UseTableOfContents) {
					const cont = document.getElementsByClassName('TOC-container')
					...
					for (const title of titles) {
						...
						title.onclick = function () {
							let urlarea = url
							if (url.lastIndexOf('#') !== -1) { urlarea = url.substring(0, url.lastIndexOf('#')) }
